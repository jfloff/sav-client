...
class SavClient:
  ...
  def remove_player_from_registration_batch(
    ...
    # in *this batch* (op=30), which answers SavRecordNotFoundError when the
    # player is not in it — the same probe resolve_batch_by_license uses to
    # detect a stale cache entry.
    #
    # An earlier version compared the batch's item_count before and after.
    # That was wrong: a concurrent removal of a *different* player also makes
    # the count fall, which would falsely confirm this one. It also could not
    ...
    self._cache.forget_license_batch(license)
    # The row's item_count/state changed, so the next caller must re-fetch
    # rather than see the pre-removal memo.
    self._invalidate_batch_memo()
    ...
    *,
    ...
    if batch.type_id == _REGISTRATIONS_TYPE_PRIMEIRA:
      ...
      return _finish(self._add_player_to_primeira_batch(
        ...
        morada=morada, cod_postal=cod_postal,
        distrito_id=distrito_id, concelho_id=concelho_id,
        localidade_txt=localidade_txt or "",
  ...
  def _commit_registration_step3(
    ...
    *,
    ...
    is_minor = _decode_sav_flag(step3_prefill.get("menor_idade"), field="menor_idade")
    if is_minor:
      missing = [
        n for n, v in [
  ...
  def _parse_option_map(html: str) -> dict[int, str]:
    ...
    SAV's address cascades answer with a bare ``<option>`` list and use ``0``
    as the "not selected" placeholder.
    """
    from bs4 import BeautifulSoup
  ...
  def _build_step2_send(
    ...
    *,
    ...
    Caller-facing locality edits use ``localidade_txt``. SAV's stored numeric
    ``localidade`` value is still carried forward internally so an unrelated
    address update cannot wipe legacy data.
    """
    def _pick(override: Any, stored: Any) -> Any:
      return override if override is not None else stored
