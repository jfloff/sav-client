...
| Term | Meaning |
|------|---------|
| **game sheet** | Pre-game eligible players list printed before tip-off. Generated by `get_eligible_players_pdf()`. |
| **referee sheet** | Post-game document uploaded to SAV2 by the referee. Downloaded by `get_game_sheet_pdf()`. |
| **licence** (licença) | Player registration number — a numeric string (e.g. `"301772"`). Used to identify players across all API calls. |
| **wallet** (carteira) | Coach/official registration number — a separate numeric identifier, distinct from player licences. Used in `coaches_pri` / `coaches_adj` fields and PDF generation. |
| **val** | Team selector passed to eligible-player methods: `1` = home team, `2` = away team. |
| **coaches_pri** | Head coaches (treinadores principais) eligible for this game. |
| **coaches_adj** | Adjunct/assistant coaches (treinadores adjuntos) eligible for this game. |
| **coaches_other** | Other coaching roles (OUTROS TREINADORES). Excluded from PDFs by default. |
| **staff** | Support staff (ENQUADRAMENTO HUMANO). Excluded from PDFs by default. |
| **tier / escalão** | Age/competition category, e.g. `"Mini 12"`, `"Sub 14"`, `"Sénior"`. Free-text string as it appears in SAV2. |
| **active** | `True` when the player is registered and eligible for the current season. The definitive eligibility signal — do not assume eligibility from presence in search results alone. |
| **association / associação** | Regional basketball body (e.g. AB Santarém). Identified by a numeric `id` from `list_associations()`. |
| **season / época** | Competition year identified by a numeric epoch ID. `None` = current season; `0` = all seasons. |
...
@dataclass(frozen=True)
class Game:
    id: int           # internal SAV2 ID — required for get_eligible_players() and get_game_sheet_pdf()
...
club. `Game.id` is required by `get_eligible_players()`, `get_eligible_players_pdf()`,
and `get_game_sheet_pdf()`.

---

### `get_eligible_players(game_id, *, val=1) → dict`

Return the eligible players list for one team (**pre-game game sheet data**). Does not generate a PDF.
...
Generate and download the **pre-game game sheet PDF** from SAV. Replicates the
...
Download the **post-game referee sheet** uploaded to SAV2 by the referee after
the game, if one exists.

> Not to be confused with `get_eligible_players_pdf()` (pre-game game sheet).
> This retrieves a document uploaded by officials; it cannot be generated on demand.

```python
pdf = client.get_game_sheet_pdf(game.id)
if pdf is not None:
    open("referee_sheet.pdf", "wb").write(pdf)
```

Returns `None` if no referee sheet has been uploaded yet.
...
### Check whether a referee sheet has been uploaded

```python
pdf = client.get_game_sheet_pdf(game.id)
has_referee_sheet = pdf is not None
```
