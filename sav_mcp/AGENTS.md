...
- `lookup_player(nif?, license?, club_id?, status="active", with_profile=false, with_details=false)` accepts exactly one of NIF or licence and uses the same ladder. `with_profile=true` adds the SAV profile under a nested `profile` key, keeping profile and roster field provenance separate.
  ...
  - **`club_id=0` is expensive and the cost is not bounded by your club's size.** `club=0` fans out one search request per club in every association — on FPB that is ~1000 requests per ladder rung, so a licence that misses every rung costs ~3000. Every fixed-season rung probes the session club first and only pays for the sweep on a miss (this applies to `get_player` too), and the all-seasons rung skips the probe so a stale own-club row can't outrank the player's current row at their new club.
  - The probe only saves you when the player is on your **current-season** roster. Right after a season rollover the current season is empty (see the season-semantics note above), so every `club_id=0` lookup misses rung 1 and pays a full sweep before the previous-season probe can hit. Measured live 2026-08-21. To avoid that guaranteed-miss sweep, `club_id=0` **skips the current-season rung entirely** while your own club's current-season roster is empty (checked once per club+epoch). Accepted trade: during that window a licence that is current at *another* club resolves to a stale own-club row from last season, because the rung-1 federation sweep is what would have found the new row. Prefer an explicit `club_id` whenever you know the club; reach for `club_id=0` only for a genuinely unknown one, e.g. an athlete transferring in.
- `warm_nif_index(scope="recent", force=false)` pre-pays NIF-index work for the session's own club in the node-local SQLite cache. It takes **no `club_id`** — the index only ever covers your own club, because SAV2 only exposes a player's NIF to their own club — and raises `ValueError` when the session club cannot be resolved. `scope="full"` is offline-only and can take minutes for a large club: it performs one profile POST per not-yet-indexed licence across all seasons, at 8-way concurrency, and will likely outlast a default MCP timeout. Use it from an importer or nightly job, not an interactive request.
