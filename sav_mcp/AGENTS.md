...
- `lookup_player(nif?, license?, club_id?, status="active", with_profile=false, with_details=false)` accepts exactly one of NIF or licence and uses the same ladder. `with_profile=true` adds the SAV profile under a nested `profile` key, keeping profile and roster field provenance separate.
  - `club_id` defaults to the session's own club. `club_id=0` searches federation-wide, but **only with `license`** — use it for a player transferring in from another club. A `nif` with `club_id=0` raises `ValueError`: **SAV2 only discloses a player's NIF to their own club**, so there is no federation-wide NIF data to search. This is a platform limit, not an indexing gap — pre-warming cannot work around it.
  - Federation-wide is not one request: `club=0` fans out one search per club in every association. Every fixed-season rung of the ladder therefore probes the session club first and only pays for the sweep on a miss (this applies to `get_player` too). The all-seasons rung skips the probe, so a stale own-club row can't outrank the player's current row at their new club.
