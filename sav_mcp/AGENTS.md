...
- `lookup_player(nif?, license?, club_id?, status="active", with_profile=false, with_details=false)` accepts exactly one of NIF or licence and uses the same ladder. `with_profile=true` adds the SAV profile under a nested `profile` key, keeping profile and roster field provenance separate.
  - `club_id` defaults to the session's own club. `club_id=0` searches federation-wide, but **only with `license`** — use it for a player transferring in from another club. A `nif` with `club_id=0` raises `ValueError`: the NIF index is built from one club's roster, so there is nothing federation-wide to search.
